...
// Helper to check if configuration is grouped by grouped into dense
...
benchdnn_dnnl_wrapper_t<dnnl_memory_desc_t> create_md(const prb_t *prb,
    ...
    if (kind == DST) {
        ...
        } else if (detect_2dby2d(prb->sparse_options)) {
...
int init_ref_memory_args(dnn_mem_map_t &ref_mem_map, dnn_mem_map_t &mem_map,
    ...
    const bool is_grouped_2dby3d = prb->sparse_options.is_grouped(DNNL_ARG_SRC)
            && prb->sparse_options.is_grouped(DNNL_ARG_DST);
    const bool is_grouped_2dby2d = detect_2dby2d(prb->sparse_options);
...
#endif
                // 2Dx2D WEI [total_K, N] is row-major (same as tag::abx),
                // so it falls through to the else path below.
                if (exec_arg == DNNL_ARG_WEIGHTS && !is_grouped_2dby2d) {
