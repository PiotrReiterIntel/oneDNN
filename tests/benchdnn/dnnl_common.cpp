...
int check_dnnl_status(dnnl_status_t status, const base_prb_t *prb, res_t *res) {
...
int init_prim(const thr_ctx_t &thr_ctx,
    ...
    std::function<int()> call_fn = std::bind(f, std::ref(user_prim),
            init_pd_func, prb, res, dir, hint, is_service_prim);
    return create_in_thr_ctx(thr_ctx, call_fn);
}

void check_correctness(const base_prb_t *prb,
