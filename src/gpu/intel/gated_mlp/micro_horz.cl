...
micro_gated_mlp_horz(const __global SRC_DATA_T *src,
    ...
    s_tile_type S_tile;
    S_tile = ugemm_wgu(AS_WTS_UP_PTR(W_up), W_UP_LD, AS_SRC_PTR(src), SRC_S0,
            OC, MB, IC, wg_i0, wg_j0, 0, sg_i_wgu, sg_j_wgu, GEMM_slm
...
#if defined ACCUM_DT_S32
    do {
        s_tile_type_f32 *S_tile_f32 = (s_tile_type_f32 *)&S_tile;
        tile_convert(S_tile, (*S_tile_f32), convert_float);
#if (SRC_SCALES == QUANTIZE_COMMON) || (WTS_UP_SCALES == QUANTIZE_COMMON)
#if SRC_SCALES == QUANTIZE_COMMON
        float src_scale = convert_float(*src_scales);
#endif
#if WTS_UP_SCALES == QUANTIZE_COMMON
        float wu_scale = convert_float(*wts_up_scales);
#endif
#if (SRC_SCALES == QUANTIZE_COMMON) && (WTS_UP_SCALES == QUANTIZE_COMMON)
#define wu_scale_op(x) ((x) * src_scale * wu_scale)
#elif WTS_UP_SCALES == QUANTIZE_COMMON
#define wu_scale_op(x) ((x) * wu_scale)
#else
#define wu_scale_op(x) ((x) * src_scale)
#endif
        tile_elementwise((*S_tile_f32), wu_scale_op);
#endif
    } while (false);
#elif defined ACCUM_DT_F32
    do {
...
#define wu_scale_op(x) ((x) * src_scale)
    ...
    S_tile = ugemm_wgu(AS_WTS_GATE_PTR(W_gate), W_GATE_LD, AS_SRC_PTR(src),
            SRC_S0, OC, MB, IC, wg_i0, wg_j0, 0, sg_i_wgu, sg_j_wgu, GEMM_slm
...
#endif
#if (SRC_SCALES == QUANTIZE_COMMON) && (WTS_GATE_SCALES == QUANTIZE_COMMON)
#define wg_scale_op(x) unary_activation((x) * src_scale * wg_scale)
#elif WTS_GATE_SCALES == QUANTIZE_COMMON
#define wg_scale_op(x) unary_activation((x) * wg_scale)
#else
#define wg_scale_op(x) unary_activation((x) * src_scale)
#endif
        tile_elementwise((*S_tile_f32), wg_scale_op);
#else
        tile_elementwise((*S_tile_f32), unary_activation);
#endif
        //sub_group_barrier(CLK_LOCAL_MEM_FENCE);
        tile_binary((*S_tile_f32), S_WU_tile, binary_mul);
    } while (false);
    s_tile_type_dst S_tile_dst;
    tile_copy_reblock((*S_tile_f32), &S_tile_dst);
#elif defined ACCUM_DT_F32
    do {
