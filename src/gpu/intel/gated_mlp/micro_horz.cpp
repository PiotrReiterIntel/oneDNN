...
//*/
    ...
    if (with_src_scales(this) && !with_common_src_scales(this)) {
        ...
        problem_wgu.bsPtrDims = (src_scales_group_size(this) > 1) ? 2 : 1;
    ...
    if (with_wts_gate_scales(this) && !with_common_wts_gate_scales(this)) {
        ...
        problem_wgu.A_scale.alignment
                = uint8_t(types::data_type_size(scale_dt));
        problem_wgu.asPtrDims = (wts_gate_scales_group_size(this) > 1) ? 2 : 1;
    ...
    if (((problem_wgu.asPtrDims > 1) || (problem_wgu.bsPtrDims > 1)
                || with_src_zp(this))
    ...
    size_t gemm_slm = gemm_gate_up_pkg().getSetting("slm_size");
    size_t kern_slm = types::data_type_size(get_accum_type()) * config.wg_m_gwu
            * config.unroll_m_gwu * config.wg_n_gwu * config.unroll_n_gwu;
    size_t slm = compute::device_info_t::max_slm_size(dev_info->gpu_product());
