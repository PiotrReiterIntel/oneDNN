...
void stream_t::before_exec_hook() {
    ...
    if (is_verbose_profiler_enabled() && !recording()) {
        auto &profiler = verbose_profiler_.get(
                utils::make_unique<xpu::sycl::verbose_profiler_t>(this));
        auto *sycl_profiler
                = static_cast<xpu::sycl::verbose_profiler_t *>(profiler.get());
        ...
        if (!recording()) {
            sycl_profiler->start_profiling();
        } else {
            if (sycl_profiler->is_active()) {
                VWARN(primitive, exec,
                        "verbose profiling is paused when sycl graph is "
                        "recording");
            }
            sycl_profiler->pause_profiling();
        }
        sycl_profiler->update_event_list();
    }
}
...
status_t stream_t::run_verbose_profiler(
    ...
    if (!vp) return status::runtime_error;
    CHECK(vp->add_to_pending_primitive_list(start_ms, pd_info));
    return status::success;
}
